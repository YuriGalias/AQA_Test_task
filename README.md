...
Work with Serenity Cucumber framework is quite easy. 
It's just enough to add Serenity dependencies (to the pom file)like:
- serenity
- serenity.maven
- serenity.cucumber (in case if we want to use cucumber) 
- serenity-rest-assured(in case if we want to create tests for API endpoints)
- maven-surefire-plugin
- maven-failsafe-plugin( both of this maven plugins needs for configuration run via Maven)
